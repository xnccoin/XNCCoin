...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90;	//stake age of full weight:  9 day
unsigned int nStakeTargetSpacing = 60;			// 60 sec block spacing

int64 nChainStartTime = 1468638349;
...
const string strMessageMagic = "XNC Signed Message:\n";
...
void CTxMemPool::queryHashes(std::vector<uint256>& vtxnc)
{
    vtxnc.clear();

    LOCK(cs);
    vtxnc.reserve(mapTx.size());
    for (map<uint256, CTransaction>::iterator mi = mapTx.begin(); mi != mapTx.end(); ++mi)
        vtxnc.push_back((*mi).first);
}
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight == 1)
        nSubsidy = 67000000 * COIN; // genesis block coinbase is unspendable
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in XNC
    bool fStrictPayToScriptHash = true; // Always active in XNC
...
if (!CheckCoinStakeTimestamp(GetBlockTime(), (int64)vtx[1].nTime))
    ...
    if ((nTime >= CUTOFF_POW_TIME) && (IsProofOfWork()))
         return DoS(100, error("CheckBlock() : Proof of work (%f XNC) at t=%d on or after %d.\n",
    ...
    // Check for duplicate txncs. This is caught by ConnectInputs(),
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "XNC", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "hellp XNC proves that pzTimestamp is overkill.";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1477157786;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 9320141; // perfect nonce
        ...
        //assert(block.hashMerkleRoot == uint256("0x8b13e82e2769cbcbd28e7f7549d87db76bb2cc327b49da16db8c11fa51034de7"));
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    {
        std::vector<uint256> vtxnc;
        mempool.queryHashes(vtxnc);
        vector<CInv> vInv;
        for (unsigned int i = 0; i < vtxnc.size(); i++) {
            CInv inv(MSG_TX, vtxnc[i]);
            vInv.push_back(inv);
            if (i == (MAX_INV_SZ - 1))
                    break;
        }
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            {
                printf("priority %.1f feeperkb %.1f txnc %s\n",
