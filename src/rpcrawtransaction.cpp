...
{
    entry.push_back(Pair("txnc", tx.GetHash().GetHex()));
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        ...
        {
            in.push_back(Pair("txnc", txin.prevout.hash.GetHex()));
...
Value getrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getrawtransaction <txnc> [verbose=0]\n"
            "If verbose=0, returns a string that is\n"
            "serialized, hex-encoded data for <txnc>.\n"
            "If verbose is non-zero, returns an Object\n"
            "with information about <txnc>.");
...
Value listunspent(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "{txnc, vout, scriptPubKey, amount, confirmations}");
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid XNC address: ")+input.get_str());
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        Object entry;
        entry.push_back(Pair("txnc", out.tx->GetHash().GetHex()));
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "createrawtransaction [{\"txnc\":txnc,\"vout\":n},...] {address:amount,...}\n"
    ...
    BOOST_FOREACH(Value& input, inputs)
    {
        ...
        const Value& txnc_v = find_value(o, "txnc");
        if (txnc_v.type() != str_type)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, missing txnc key");
        string txnc = txnc_v.get_str();
        if (!IsHex(txnc))
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, expected hex txnc");
        ...
        CTxIn in(COutPoint(uint256(txnc), nOutput));
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid XNC address: ")+s.name_);
...
Value signrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 4)
        throw runtime_error(
            "signrawtransaction <hex string> [{\"txnc\":txnc,\"vout\":n,\"scriptPubKey\":hex},...] [<privatekey1>,...] [sighashtype=\"ALL\"]\n"
    ...
    if (params.size() > 1 && params[1].type() != null_type)
    {
        ...
        {
            if (p.type() != obj_type)
                throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "expected object with {\"txnc'\",\"vout\",\"scriptPubKey\"}");

            Object prevOut = p.get_obj();

            RPCTypeCheck(prevOut, map_list_of("txnc", str_type)("vout", int_type)("scriptPubKey", str_type));

            string txncHex = find_value(prevOut, "txnc").get_str();
            if (!IsHex(txncHex))
                throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "txnc must be hexadecimal");
            uint256 txnc;
            txnc.SetHex(txncHex);
...
string sendtoaddresswithtime(string sAddress, int64 nAmount, unsigned int nTime) {
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid XNC address");
